...
int help(int error)
{
    printf("Usage: %s -c COMMAND -d DIRECTORY [-e event,[event,[,..]]] [-n] [-r] [-a] [-v] [-s]\n", PROGRAM_NAME);
    printf("or   : %s [-V|--version]\n", PROGRAM_NAME);
    printf("or   : %s [-h|--help]\n\n", PROGRAM_NAME);
    printf("  -c --command COMMAND\n");
    printf("     The command to execute when an event occurs.\n");
    printf("     It is possibile to pass to the command special arguments generated by cwatch:\n");
    ...
    printf("  -V  --version\n");
    printf("      Out the version of the program and exit\n\n");
...
int parse_command_line(int argc, char *argv[])
{
    ...
    int c;
    while ((c = getopt_long(argc, argv, "svnrVhe:c:d:x:", long_options, NULL)) != -1) {
        switch (c) {
        case 'c': /* --command */
            if (optarg == NULL
                || strcmp(optarg, "") == 0
        ...
        case 'x': /* --exclude */
            ...
            user_regex = (regex_t *) malloc(sizeof(regex_t));
            
            if (regcomp(user_regex, optarg, REG_EXTENDED | REG_NOSUB) != 0) {
                free(user_regex);
                help(0);
...
bool_t excluded(char *str)
{
    ...
    if (regexec(user_regex, str, 0, NULL, 0) == 0)
        return TRUE;
